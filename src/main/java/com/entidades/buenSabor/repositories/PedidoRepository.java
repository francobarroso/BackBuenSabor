...
public interface PedidoRepository extends BaseRepository<Pedido,Long>{
    ...
    List<Pedido> findBySucursalIdOrderByIdDesc(Long idSucursal);

    @Query("SELECT p FROM Pedido p WHERE p.fechaPedido BETWEEN :startDate AND :endDate AND p.estado = com.entidades.buenSabor.domain.enums.Estado.ENTREGADO AND p.sucursal.id = :idSucursal")
    List<Pedido> findPedidoByDate(@Param("startDate") LocalDate startDate,
                                           @Param("endDate") LocalDate endDate,
                                  @Param("idSucursal") Long idSucursal);

    @Query("SELECT SUM(p.total) FROM Pedido p WHERE p.fechaPedido BETWEEN :startDate AND :endDate AND p.estado = com.entidades.buenSabor.domain.enums.Estado.ENTREGADO AND p.sucursal.id = :idSucursal")
